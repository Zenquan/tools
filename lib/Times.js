class Times {
   ...
   */
    ...
    getMonths(time, len, direction) {
        let mm = new Date(time).getMonth(), yy = new Date(time).getFullYear(), directions = isNaN(direction) ? 3 : direction, index = mm;
        ...
        let formatNext = function (i) {
            let y = Math.floor(i / 12), m = i % 12;
            return [yy + y + '-' + (m + 1)];
        };
        let formatPre = function (i) {
            let y = Math.ceil(i / 12), m = i % 12;
            m = m === 0 ? 12 : m;
            return [yy - y + '-' + (13 - m)];
        };
        let formatCurr = function (i) {
            let y = Math.floor(i / 12), yNext = Math.ceil(i / 12), m = i % 12, mNext = m === 0 ? 12 : m;
